...
function resetGame() {
    ...
    visibleOptions.forEach((div, index) => {
        ...
        div.classList.remove('disabled', 'try-again', 'hints', 'correct', 'super-button');
        div.style.pointerEvents = 'auto';
        div.style.display = index < currentDifficulty ? 'block' : 'none';
    });
    // Remove super button if exists
        ...
        if (visibleIndex === correctIndex) {
            ...
            if (clickCount === 1) {
                ...
                if (isMobilePortrait()) {
                    ...
                    superButton.innerText = hint;
                    superButton.addEventListener('click', () => {
                        superButton.classList.add('correct');
                        setTimeout(resetGame, 1000);
                    });
                    document.getElementById('rightSide').appendChild(superButton);
                } else {
                    // Landscape/others: Show hints in incorrect buttons
