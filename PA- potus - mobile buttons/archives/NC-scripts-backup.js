...
function saveData() {
    let data = presidents.map(p => ({
        ...
        corrects: p.corrects,
        usedHints: p.usedHints || [] // Save used hints
    }));
...
function startNewRound() {
    ...
    // Ensure full randomization for every round
    order = shuffle(order);
    if (round >= 4) {
    ...
    div.addEventListener('click', function() {
        ...
        if (isCorrect) {
            ...
            if (attempts === 2) {
                ...
                // Step 2: Prepare two sentences with cycling logic
                let sentences = [];
                if (currentPresident.name === "Garfield") {
                    ...
                    let termAccomplishments = correctAccomplishments.filter(entry => 
                        entry.year >= termYears.start && entry.year <= termYears.end
                    );
                    // Initialize usedHints for Cleveland if not already set
                    if (!currentPresident.usedHints) {
                        currentPresident.usedHints = [];
                    }
                    // Filter out used hints for this term
                    let availableHints = termAccomplishments.filter(hint => 
                        !currentPresident.usedHints.includes(hint.description)
                    );
                    if (availableHints.length === 0) {
                        // Reset used hints if all have been used
                        currentPresident.usedHints = [];
                        availableHints = termAccomplishments;
                    }
                    // Select one hint for this term
                    let hint1 = availableHints[Math.floor(Math.random() * availableHints.length)];
                    sentences[0] = hint1.description;
                    // Mark the hint as used
                    currentPresident.usedHints.push(hint1.description);
                    // Second hint is always the non-consecutive terms fact
                    sentences[1] = "Grover Cleveland served two non-consecutive terms as the 22nd and 24th President.";
                } else {
                    // For other presidents, implement cycling logic
                    if (!currentPresident.usedHints) {
                        currentPresident.usedHints = [];
                    }
                    ...
                    if (availableHints.length < 2) {
                        // Reset used hints if not enough available
                        currentPresident.usedHints = [];
                        availableHints = correctAccomplishments;
                    }
                    // Select two unique hints
                    let hint1 = availableHints[Math.floor(Math.random() * availableHints.length)];
                    let remainingHints = availableHints.filter(h => h !== hint1);
                    let hint2 = remainingHints.length > 0 ? 
                        remainingHints[Math.floor(Math.random() * remainingHints.length)] : hint1;
                    sentences = [hint1.description, hint2.description];
                    // Mark hints as used
                    currentPresident.usedHints.push(hint1.description);
                    if (hint2 !== hint1) {
                ...
                optionDivs.forEach(d => {
                    ...
                    if (isCorrectOption) {
                        ...
                        incorrectIndex += 1; // Move to next sentence
                    }
                });
